...
- [ ] EV Cable Lock: Fix the button, state and icon so they correctly show and control Locked / Unlocked status.
- [ ] Charging Current: Fix reading and UI synchronization so the set charging current is correctly displayed and updated in both directions.
- [ ] Initial Vehicle State: Add a proper initial state after connecting a vehicle, optionally requiring RFID activation to identify the private car or an external user.
- [ ] Charging Profiles: Support different charging profiles, e.g. full 11 kW power for an authorized external user.
- [ ] Energy Accounting: Record consumed grid energy and PV energy separately for accurate charging-cost calculation.
- [ ] Shared Charging: Add RFID-based user identification and charging-cost calculation for external users, including configurable fees or commission.
